/**
 * C4 Person Node Component
 *
 * Renders a Person node for C4 model diagrams following official C4 notation.
 * - Uses a person icon/stick figure representation
...
import { User, ArrowDownRight } from 'lucide-react';
...
interface C4PersonNodeData extends NodeData {
  /** C4-specific metadata */
  c4Metadata?: C4ElementMetadata;
}

const C4PersonNode = memo(({ id, data, selected }: NodeProps<C4PersonNodeData>) => {
  ...
  const sanitizedHtml = useMemo(() => {
    ...
    return DOMPurify.sanitize(data.htmlContent, {
      ALLOWED_TAGS: ['div', 'span', 'p', 'strong', 'em', 'u', 'br', 'hr', 'small', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li', 'img', 'a', 'b', 'i', 'code', 'pre', 'blockquote', 'table', 'tr', 'td', 'th', 'thead', 'tbody'],
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          height: '40px',
          borderRadius: '50%',
          backgroundColor: isExternal ? '#e2e8f0' : '#ecfeff',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          color: isExternal ? '#64748b' : '#0891b2',
        }}
      >
